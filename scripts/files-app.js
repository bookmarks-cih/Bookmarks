// Hamburger menu elements
  ...
  const filesDataset = window.FILES_DATA || [];
  ...
  const App = {
    ...
    renderGrid: function() {
      ...
      if (filtered.length === 0) {
        grid.innerHTML = '<p class="empty-state" style="grid-column: 1/-1; text-align: center; padding: 40px; color: var(--text-dim);">No resources found matching your criteria.</p>';
        return;
      }
    ...
    getCategories: function(data) {
      ...
      const preferredOrder = ['Torrents', 'Streaming', 'Games', 'Software', 'VST', 'PDF', 'Design', 'DDL', 'Tracker', 'Scene', 'Anime', 'Archive', 'Dev'];
    ...
    init: function() {
      initTheme();
      this.state.rawData = filesDataset;
